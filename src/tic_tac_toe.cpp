...
ACTION tic_tac_toe::create(name challenger, name host) {
  require_auth(host);
  check(challenger != host, "create: challenger shouldn't be the same as host");

  games existing_host_games(_self, host.value);
  auto itr = existing_host_games.find(challenger.value);
  check(itr == existing_host_games.end(), "create: game already exists");
...
ACTION tic_tac_toe::restart(name challenger, name host, name by) {
  ...
  games existing_host_games(_self, host.value);
  auto itr = existing_host_games.find(challenger.value);
  check(itr != existing_host_games.end(), "restart: game doesn't exists");

  check(by == itr->host || by == itr->challenger, "restart: this is not your game!");
...
ACTION tic_tac_toe::close(name challenger, name host) {
  ...
  games existing_host_games(_self, host.value);
  auto itr = existing_host_games.find(challenger.value);
  check(itr != existing_host_games.end(), "close: game doesn't exists");
...
bool is_valid_movement(const uint16_t& row,
  ...
  bool is_valid =
      column < board_width && row < board_height && is_empty_cell(board[movement_location]);
  return is_valid;
}
...
ACTION tic_tac_toe::move(name challenger,
                         name host,
                         name by,
                         const uint16_t row,
                         const uint16_t column) {
  ...
  games existing_host_games(_self, host.value);
  auto itr = existing_host_games.find(challenger.value);
  check(itr != existing_host_games.end(), "move: game doesn't exists");
  check(itr->winner == "none"_n, "move: the game has ended!");
  check(by == itr->host || by == itr->challenger, "move: this is not your game!");
  check(by == itr->turn, "move: it's not your turn yet!");
  check(is_valid_movement(row, column, itr->board), "move: not a valid movement!");
